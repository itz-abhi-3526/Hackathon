-- ═══════════════════════════════════════════════════════════════
--   HACK2PITCH 2026 — FINAL PER-TEAM-SIZE PRICING
--
--   `registration_rounds.fee` has ALREADY been removed from the
--   database. This migration makes the remaining database consistent
--   with that: it repairs every function/trigger that still referenced
--   the deleted column, so the active-round API and the registration
--   write path keep working with ONLY:
--
--       fee_2_members / fee_3_members / fee_4_members
--
--   It does:
--     1. drops the obsolete BEFORE-INSERT round-stamp trigger, which
--        read `v_round.fee` and would now fail on every teams INSERT
--     2. drops the obsolete submit_registration() overloads (the
--        canonical register_team is self-contained and never calls them)
--     3. recreates public_active_round()  — returns the 3 per-size
--        fees, NO `fee` key, same fields + security + grants
--     4. recreates register_team(jsonb)  — authoritative fee =
--        active round + participant count from fee_2/3/4_members only,
--        rejected when NULL/<=0, any client-supplied fee ignored.
--        Signature, SECURITY DEFINER, search_path, capacity (TEAMS),
--        row lock and auto-close are all preserved.
--     5. reloads the PostgREST schema cache
--
--   teams.registration_fee is UNTOUCHED (immutable historical snapshots).
--   One transaction. Safe to re-run. No reference to `fee` anywhere.
-- ═══════════════════════════════════════════════════════════════
begin;

-- ── 1. Obsolete round-stamp trigger (read the removed column) ────
--      register_team now stamps registration_round_id +
--      registration_fee on every insert itself.
...
-- ── 2. Obsolete submit_registration() overloads ──────────────────
...
-- ── 4. register_team(payload jsonb): the AUTHORITATIVE price ─────
--    Signature unchanged; callable exactly as the frontend calls it:
--    supabase.rpc('register_team', { payload: {...} }). The fee is
--    derived ONLY from the locked active round + the participant count
--    using the per-size columns. The client-supplied fee is ignored.
--    A NULL/non-positive price rejects the registration.
...
begin
  ...
  /* ── THE AUTHORITATIVE FEE — active round + participant count only.
        The per-size column of the ACTIVE round decides the amount.
        The browser never supplies it; any value sent in the payload is
        ignored. A NULL or non-positive price rejects the registration.
        Stored on teams.registration_fee BEFORE the team row commits. ── */
  case v_count
