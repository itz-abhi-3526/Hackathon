-- ═══════════════════════════════════════════════════════════════
--   HACK2PITCH 2026 — TEAM SIZE 3 OR 4 ONLY (NEW REGISTRATIONS)
--
--   Official rule: a HACK2PITCH 2026 team MUST contain 3 or 4
--   members. Teams of 2 are NO LONGER ALLOWED for new registrations.
--
--   WHAT THIS DOES
--     1. Recreates public.register_team(payload jsonb) — the single
--        canonical submit RPC (same signature, same body) tightening
--        the participant-count gate to:
--
--            before:  2 <= participant_count <= 4   (previous)
--            after:   3 <= participant_count <= 4   (official)
--
--        The participant JSONB array IS the team size (there is no
--        separate team_size column), so this single check still
--        rejects every invalid registration (0/1/2 or >4 participants,
...
--        teams — are NOT modified, migrated, or deleted. They remain
--        intact and visible in admin.
--
--     3. Does NOT change fees, rounds, capacity, payment proof flow,
--        or the RPC signature. Fee selection (fee_2/3/4_members) is
--        unchanged; with the count gate above, v_count is 3 or 4.
--
...
begin
  ...
  /* OFFICIAL TEAM-SIZE RULE: 3-member and 4-member teams register. */
...
--    admin. Only NEW registrations must have exactly 3 or 4 members.
