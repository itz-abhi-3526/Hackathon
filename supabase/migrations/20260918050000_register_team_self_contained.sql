-- ═══════════════════════════════════════════════════════════════
--   HACK2PITCH 2026 — register_team(payload jsonb): SELF-CONTAINED
--
--   PROVEN root cause of the submit-time HTTP 400:
--
...
--   stamps the fee with `v_round.fee` (a registration_rounds%rowtype
--   field). The generic `fee` column was removed, so that reference
--   dies at runtime → 42703 → PostgREST 400.
--
--   This migration replaces it with the single canonical
--
--     public.register_team(payload jsonb)
--
--   which is FULLY SELF-CONTAINED (no submit_registration call):
--
--     1. resolves the ACTIVE round  (status = 'active',
--        order by created_at desc, id desc, limit 1)
--     2. row-locks it FOR UPDATE, re-checks status + window
--     3. validates the problem statement against problem_statements
--     4. validates participants (array, 2-4 members, exactly one lead)
--     5. computes the AUTHORITATIVE fee from the participant count:
--        2 → fee_2_members | 3 → fee_3_members | 4 → fee_4_members
--        (client fee/registration_fee/amount/price ignored; NULL or
--        <= 0 fee rejects)
--     6. enforces capacity counting TEAMS (auto-closes round at limit)
--     7. inserts the team with registration_round_id + registration_fee
--     8. inserts participants (existing columns + lead/member semantics)
--     9. returns { team_id, registration_code, payment_status,
--                 submitted_at, registration_round_id,
--                 registration_fee, round_title }
--
--   It also drops the obsolete round-stamp trigger (which read the
--   removed `fee` column and would break every teams INSERT) and the
--   legacy submit_registration() overloads (register_team no longer
--   calls them). It does NOT recreate public_active_round().
--
--   postgres NOT NULL / check constraints on teams and participants are
--   honoured (they are defaults from the original schema). One
--   transaction, safe to re-run, schema cache reloaded at the end.
-- ═══════════════════════════════════════════════════════════════
begin;

-- ── 1. Obsolete round-stamp trigger (referenced the removed column) ─
--      register_team stamps registration_round_id + registration_fee
--      on every insert itself.
...
-- ── 3. THE canonical, self-contained register_team(payload jsonb) ──
...
-- ── 4. Make the schema changes discoverable by the deployed app ──
