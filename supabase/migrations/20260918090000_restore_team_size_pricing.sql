-- ═══════════════════════════════════════════════════════════════
--   HACK2PITCH 2026 — RESTORE TEAM-SIZE PRICING + FINAL SUBMIT (one shot)
--
--   PROVEN live root cause (verified via the REST API):
--     POST /rest/v1/rpc/register_team  →  HTTP 400
--     {"code":"42703","message":"record \"v_round\" has no field \"fee\""}
--
--   The DEPLOYED register_team is the legacy version whose body still
--   stamps registration_fee from `v_round.fee`. The generic `fee`
--   column has ALREADY been removed from `registration_rounds`
--   (the live public_active_round() returns fee_2/3/4_members), so
--   the stale body dies at runtime with 42703 → PostgREST 400.
--
--   THIS FILE, applied in ONE transaction, restores the flow using
--   ONLY the three per-team-size columns:
--
--     registration_rounds.fee_2_members
--     registration_rounds.fee_3_members
--     registration_rounds.fee_4_members
--
--   It does NOT recreate `fee`, does NOT reference `fee` anywhere, and
--   does NOT touch historical teams/participants rows, capacity
--   semantics, problem_statements, admin tables or any other object.
--
--   WHAT IT DOES
--     1. Drops the obsolete BEFORE-INSERT round-stamp trigger (it read
--        `v_round.fee`, so it would break EVERY teams INSERT).
--     2. Drops the legacy submit_registration() overloads — the
--        canonical register_team is fully self-contained and never
--        calls them.
--     3. Recreates public.public_active_round()  — zero parameters.
--        Returns id / title / slug / capacity / status / starts_at /
--        ends_at / fee_2_members / fee_3_members / fee_4_members plus
--        live registered / remaining / open availability. NO `fee`.
--     4. Recreates public.register_team(payload jsonb) — the single
--        canonical submit. Fully self-contained (no submit_registration
--        dependency). The fee is derived ONLY from the ACTIVE round and
--        the participant count (2/3/4 → fee_2/3/4_members); NULL or
--        <= 0 configured price rejects the entry; any client-supplied
--        fee/amount/price is ignored. Capacity counts TEAMS and the
--        round auto-closes at capacity under a row lock.
--     5. NOTIFY pgrst, 'reload schema' so the app sees the new
...
-- ── 1. Obsolete round-stamp trigger (referenced the removed column) ─
drop trigger if exists trg_teams_stamp_active_round on public.teams;
drop function if exists public.stamp_active_round_on_teams();

-- ── 2. Legacy submit_registration() overloads — no longer called ────
...
drop function if exists public.submit_registration() cascade;
drop function if exists public.submit_registration(jsonb) cascade;

-- ── 3. public_active_round(): the 3 per-size fees, NO legacy fee ────
...
begin
  ...
  return jsonb_build_object(
    ...
    'fee_2_members', v_round.fee_2_members,
    'fee_3_members', v_round.fee_3_members,
    'fee_4_members', v_round.fee_4_members,
...
comment on function public.public_active_round() is
  'Public read of the current active registration round + live availability + the three per-team-size fees (fee_2_members / fee_3_members / fee_4_members). The legacy single fee is removed. Returns {} when registration is closed.';

-- ── 4. register_team(payload jsonb): the ONE canonical submit ───────
--    Frontend call: supabase.rpc('register_team', { payload: {...} })
--    Fully self-contained (never calls submit_registration). The
--    AUTHORITATIVE fee = ACTIVE round + participant count, from the
--    per-size columns ONLY. Client fee/amount/price is ignored.
--    NULL/non-positive configured price rejects. Capacity counts TEAMS.
...
-- ── 5. Make the schema changes discoverable by the deployed app ──
